...
describe('PATCH /todos/:id', () => {
    ...
    it('should not update the todo created by other', (done) => {
        ...
        request(app)
            .patch(`/todos/${id}`)
            .set('x-authfrank', users[1].tokens[0].token)
            .send({ text: newText, completed: true })
            .expect(404)
            .end((err, res) => {
                if (err) return done(err);
                ...
                Todo.find({ _id: id }).then((todos) => {
                    expect(todos.length).toBe(1);
                    expect(todos[0].text).toNotBe(newText);
                    done();
                }).catch((e) => done(e));
            });
...
describe('POST /users/login', () => {
        ...
        request(app)
            ...
            .end((err, res) => {
                ...
                User.findById(users[1]._id).then((user) => {
                    expect(user.tokens[1]).toInclude({ access: 'authFrank', token: res.headers['x-authfrank'] });
                    done();
                }).catch((e) => done(e));
            });
    });
    ...
    it('should reject invalid login', (done) => {
        request(app)
            .post('/users/login')
            .send({ email: users[1].email, password: 'test' })
            .expect(400)
            .expect((res) => {
                expect(res.headers['x-authfrank']).toNotExist();
            })
            .end((err, res) => {
                if (err) return done(err);
